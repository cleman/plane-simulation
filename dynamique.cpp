...
Forces compute_forces(State *etat, Actionneur cmd) {
    ...
    double temp = rho_air*pow(etat->twist_vec(0), 2)/2;
    for (int i = 0; i < 4; i++) {
        forces.lift[i] = temp*coef_aile.C_L[i]*S[i/2];
        forces.drag[i] = temp*coef_aile.C_D[i]*S[i/2];
    }
...
Dynamique compute_acc_moment(State etat, Forces forces) {
    ...
    // Print the moment vector
    //cout << "Moment vector: " << dyn.moment_vec << endl;
